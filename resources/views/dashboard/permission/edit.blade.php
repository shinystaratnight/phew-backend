...
@endsection
@section('page_title')
{{ trans('dash.permissions.edit_permission_data') }}
@endsection
@section('sidebar_title')
    <a href="{{ route('dashboard.permission.index') }}" class="breadcrumb-item">{{ trans('dash.permissions.permissions') }}</a>
    <span class="breadcrumb-item active">{{ trans('dash.permissions.edit_permission_data') }}</span>
@endsection
@section('content')
    ...
    <div class="col-md-12">
        ...
        <div class="card">
            <div class="card-header header-elements-inline">
                <h5 class="card-title">{{ trans('dash.permissions.edit_permission_data') }} : {{ $role['role_' . app()->getLocale()] }}</h5>                        
            </div>
            <div class="card-body">
                <form action="{{ route('dashboard.permission.update', $role->id) }}" method="POST">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
